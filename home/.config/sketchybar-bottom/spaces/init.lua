...
sketchybar.exec("yabai -m query --displays", function(displays)
  ...
  for _, value in pairs(displays) do
    total_spaces = total_spaces + #value.spaces
  end
...
local on_space_changed = function(env)
...
sketchybar
  .add("item", "space_watcher", {
    drawing = false,
  })
  :subscribe("front_app_switched", on_space_changed)
